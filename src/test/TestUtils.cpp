...
modifySorobanNetworkConfig(Application& app,
                           ...
                           std::shared_ptr<TestAccount> rootPtr)
{
    if (!modifyFn)
    {
        return;
    ...
    if (!rootPtr)
    {
        rootPtr = std::make_shared<TestAccount>(TestAccount::createRoot(app));
    }
    ...
    auto createUpgradeLoadGenConfig = GeneratedLoadConfig::txLoad(
        LoadGenMode::SOROBAN_CREATE_UPGRADE, 1, 1, 1);
    auto cfg = app.getLedgerManager().getLastClosedSorobanNetworkConfig();
    modifyFn(cfg);
    createUpgradeLoadGenConfig.copySorobanNetworkConfigToUpgradeConfig(cfg);
    ...
    auto cfg2 = app.getLedgerManager().getLastClosedSorobanNetworkConfig();
