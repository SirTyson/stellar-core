...
TEST_CASE_VERSIONS("eviction scan", "[bucketlist]")
{
    ...
    auto test = [&](Config& cfg) {
        ...
        bool tempOnly = protocolVersionIsBefore(
            cfg.TESTING_UPGRADE_LEDGER_PROTOCOL_VERSION, ProtocolVersion::V_22);
        ...
        auto checkArchivedBucketList = [&] {
            if (!tempOnly)
            {
                ...
                {
                    auto archivedEntry = archiveSnapshot->getArchiveEntry(k);
                    ...
                    auto archivedTTL = archiveSnapshot->getArchiveEntry(ttl);
                    REQUIRE(!archivedTTL);
                }
