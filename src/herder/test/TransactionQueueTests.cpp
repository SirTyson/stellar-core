                   ...
                   "[herder][transactionqueue]")
{
    ...
    for_versions_from(19, *app, [&] {
        ...
        SECTION("remove invalid ledger bound after close")
        {
            ...
            auto payload = herder.recvTransaction(tx, false);
            REQUIRE(payload.statusCode ==
                    TransactionQueue::AddResult::ADD_STATUS_PENDING);
            REQUIRE(!payload.txResult);
            REQUIRE(payload.sorobanDiagnostics.empty());
          ...
          "[soroban][transactionqueue]")
{
    ...
    REQUIRE(app->getHerder().recvTransaction(tx, false).statusCode ==
            TransactionQueue::AddResult::ADD_STATUS_ERROR);
          ...
          "[herder][transactionqueue][soroban]")
{
    ...
    SECTION("malformed tx")
    {
        ...
        auto badPayload = app->getHerder().recvTransaction(badTx, false);
        REQUIRE(badPayload.statusCode ==
                TransactionQueue::AddResult::ADD_STATUS_ERROR);
        REQUIRE(badPayload.txResult->result.code() == txMALFORMED);
    }
    SECTION("source account limit, soroban and classic tx queue")
    {
        ...
        auto checkLimitEnforced = [&](auto const& pendingTx,
            ...
            REQUIRE(
                app->getHerder().recvTransaction(pendingTx, false).statusCode ==
                TransactionQueue::AddResult::ADD_STATUS_PENDING);

            // Can't submit tx due to source account limit
            REQUIRE(app->getHerder()
                        .recvTransaction(rejectedTx, false)
                        .statusCode ==
                    TransactionQueue::AddResult::ADD_STATUS_TRY_AGAIN_LATER);
    ...
    SECTION("tx does not fit")
    {
        SECTION("reject")
        {
            ...
            REQUIRE(app->getHerder().recvTransaction(txNew, false).statusCode ==
                    TransactionQueue::AddResult::ADD_STATUS_PENDING);
            ...
            SECTION("insufficient fee")
            {
                ...
                auto payload2 = app->getHerder().recvTransaction(tx2, false);
                REQUIRE(payload2.statusCode ==
                        TransactionQueue::AddResult::ADD_STATUS_ERROR);
                REQUIRE(!app->getHerder().isBannedTx(tx->getFullHash()));
                REQUIRE(payload2.txResult->result.code() ==
                        TransactionResultCode::txINSUFFICIENT_FEE);
                REQUIRE(payload2.txResult->feeCharged == expectedFeeCharged);
            }
        ...
        SECTION("accept but evict first tx")
        {
            ...
            auto status =
                app->getHerder().recvTransaction(tx2, false).statusCode;
            REQUIRE(status == TransactionQueue::AddResult::ADD_STATUS_PENDING);
            auto status2 =
                app->getHerder().recvTransaction(tx3, false).statusCode;
...
TEST_CASE("remove applied", "[herder][transactionqueue]")
{
    ...
    REQUIRE(tq.getTransactions({}).size() == 1);
    REQUIRE(herder.recvTransaction(tx4, false).statusCode ==
            TransactionQueue::AddResult::ADD_STATUS_PENDING);
