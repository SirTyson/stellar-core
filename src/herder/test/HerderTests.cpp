...
TEST_CASE("getInvalidTxListWithErrors returns no duplicates", "[txset]")
{
    Config cfg(getTestConfig());
    VirtualClock clock;
    Application::pointer app = createTestApplication(clock, cfg);
          ...
          "[txset]")
{
    ...
    // Worker scheduling must not change the existing shared-fee policy or
    // the ordering of validation errors followed by fee errors.
    auto checkInvalidTxs = [&](TxFrameList const& txs,
        ...
        {
            CAPTURE(workers);
            app->getBatchExecutor().setPreferredTaskCountForTesting(workers);
            auto fees = accountFeeMap;
            auto const [invalidTxs, result] =
                TxSetUtils::getInvalidTxListWithErrors(txs, *app, fees, 0, 0);
    ...
    SECTION("fee bumps with a shared fee source")
    {
        ...
        checkInvalidTxs({fb1, fb2}, {}, {fb1, fb2},
                        TxSetValidationResult::ACCOUNT_CANT_PAY_FEE);
        checkInvalidTxs({invalidFb, fb1, fb2}, {}, {invalidFb, fb1, fb2},
                        TxSetValidationResult::TX_VALIDATION_FAILED);
        checkInvalidTxs({fb2, invalidFb, fb1}, {}, {invalidFb, fb2, fb1},
                        TxSetValidationResult::TX_VALIDATION_FAILED);
        checkInvalidTxs({fb1, fb2, invalidFb}, {}, {invalidFb, fb1, fb2},
                        TxSetValidationResult::TX_VALIDATION_FAILED);
    }
    ...
    SECTION("fee bumps mixed with regular txs")
    {
        ...
        checkInvalidTxs({tx, fb}, {}, {tx, fb},
                        TxSetValidationResult::ACCOUNT_CANT_PAY_FEE);
        checkInvalidTxs({fb, tx}, {}, {fb, tx},
                        TxSetValidationResult::ACCOUNT_CANT_PAY_FEE);
    }
